...
covid_country_ets <- covid_ets  %>% 
  ...
  mutate(
    ...
    active_lag14 = dplyr::lag(confirmed - deaths - recovered, 14, 0),
    active_lag7 = dplyr::lag(confirmed - deaths - recovered, 7, 0),
...
covid_country_ets$recovered_na <- covid_country_ets$recovered
covid_country_ets$recovered_na[covid_country_ets$recovered_na == 0] <- NA
...
cases_7_day_model <- lm(lag_7_elasticity ~ days_since_50_cases,data = seven_day_elasticity_by_country_no_outliers)
...
death_models <- covid_country_ets %>% group_by(Country.Region) %>% do(model = lm(deaths ~ active_lag14  -1, data = .))
deaths_tidy_model <- death_models %>% tidy(model)

recovered_models <- covid_country_ets %>% group_by(Country.Region) %>% do(model = lm(recovered ~ active_lag14 -1, data = .))
recovered_tidy_model <- recovered_models %>% tidy(model)
