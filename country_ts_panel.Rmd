...
daily_vector <- seq(from = as.Date("2020-01-22"), (today()-1), "day")
daily_vector <- format(daily_vector, "%m-%d-%Y")

for(i in 1:length(daily_vector)) {
  day_get <- daily_vector[i]
  if(i==1) {
    hopkins_df <- data.frame(date = day_get, read.csv(paste0("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/", daily_vector[i],".csv")), stringsAsFactors = FALSE)
    colnames(hopkins_df) <- gsub("\\.", "_", colnames(hopkins_df))
    } else {
      df <- data.frame(day = day_get, read.csv(paste0("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/", daily_vector[i],".csv")), stringsAsFactors = FALSE)
...
covid_ets <- x %>% group_by( Country.Region, Province.State, date) %>% summarise(confirmed = sum(confirmed, na.rm=TRUE), deaths = sum(deaths, na.rm=TRUE))
...
covid_country_ets <- covid_country_ets %>% filter(confirmed >= 100)
covid_country_ets <- covid_country_ets %>% group_by(Country.Region) %>% mutate(days_since_50 = dplyr::row_number())


covid_country_ets$deaths_na <- covid_country_ets$deaths
covid_country_ets$deaths_na[covid_country_ets$deaths_na == 0] <- NA
covid_country_ets$confirmed_na <- covid_country_ets$confirmed
covid_country_ets$confirmed_na[covid_country_ets$confirmed_na == 0] <- NA
covid_country_ets$days_since_50_na <- covid_country_ets$days_since_50
covid_country_ets$days_since_50_na[covid_country_ets$days_since_50_na == 0] <- NA
covid_country_ets$active_lag14_na <- covid_country_ets$active_lag14
covid_country_ets$active_lag14_na[covid_country_ets$active_lag14_na == 0] <- NA
covid_country_ets$active_lag7_na <- covid_country_ets$active_lag7
covid_country_ets$active_lag7_na[covid_country_ets$active_lag7_na == 0] <- NA
covid_country_ets$confirmed_lag7_na <- covid_country_ets$confirmed_lag7
covid_country_ets$confirmed_lag7_na[covid_country_ets$confirmed_lag7_na == 0] <- NA
covid_country_ets$confirmed_lag14_na <- covid_country_ets$confirmed_lag14
covid_country_ets$confirmed_lag14_na[covid_country_ets$confirmed_lag14_na == 0] <- NA
covid_country_ets$new_confirmed_lag7_na <- covid_country_ets$new_confirmed_lag7
covid_country_ets$new_confirmed_lag7_na[covid_country_ets$new_confirmed_lag7_na == 0] <- NA
covid_country_ets$new_confirmed_lag14_na <- covid_country_ets$new_confirmed_lag14
covid_country_ets$new_confirmed_lag14_na[covid_country_ets$new_confirmed_lag14_na == 0] <- NA
covid_country_ets$new_confirmed_na <- covid_country_ets$new_confirmed
covid_country_ets$new_confirmed_na[covid_country_ets$new_confirmed_na == 0] <- NA

days_since_50_by_country <- covid_country_ets %>% group_by(Country.Region) %>% summarize(days_since_50 = max(days_since_50))

confirmed_models <- covid_country_ets %>% group_by(Country.Region) %>% do(model = lm(log(new_confirmed_na) ~  log(new_confirmed_lag7_na) -1, data = .))
confirmed_tidy_model <- confirmed_models %>% tidy(model)

seven_day_elasticity_by_country <- data.frame(country = confirmed_tidy_model$Country.Region, days_since_50_cases  = as.numeric(days_since_50_by_country$days_since_50), lag_7_elasticity = confirmed_tidy_model$estimate)


cases_7_day_model <- lm(lag_7_elasticity ~ days_since_50_cases,data = seven_day_elasticity_by_country)

seven_day_elasticity_by_country$prediction <-  cases_7_day_model$coefficients[[1]] + cases_7_day_model$coefficients[[2]] * seven_day_elasticity_by_country$days_since_50_cases
...
require(ggrepel)
ggplot(seven_day_elasticity_by_country_no_outliers, aes(x=days_since_50_cases)) +
    geom_point(shape=1, show.legend = FALSE, (aes(y=lag_7_elasticity, color = country))) +    # Use hollow circles
      geom_line(aes( y = prediction)) +
   geom_label_repel(data=  seven_day_elasticity_by_country_no_outliers, aes(x=days_since_50_cases, y=lag_7_elasticity, label = country), size=2) + 
    theme(legend.position = "none") 

knitr::kable(seven_day_elasticity_by_country_no_outliers %>% mutate(residual = lag_7_elasticity - prediction) %>% arrange(days_since_50_cases, residual), caption = "Countries by Predicted vs. Actual Lag 7 New Case Elasticity on Today's Cases")

actuals <- covid_country_ets %>% select(country = Country.Region, date, days_since_50_cases = days_since_50, confirmed, new_confirmed, new_confirmed_lag7)

prediction <- expand.grid(country = unique(seven_day_elasticity_by_country$country), days_since_50_cases = 1:100) %>% 
  arrange(country, days_since_50_cases)

prediction$prediction <- predict.lm(cases_7_day_model, prediction)

prediction <- merge(prediction, actuals, on.x = c("country", "days_since_50_cases"), all.x = TRUE, all.y = FALSE)
...
covid_ets_state <- covid_ets_state %>% group_by(Province.State) %>% mutate(days_since_50 = dplyr::row_number())
...
days_since_50_by_state <- covid_ets_state %>% group_by(state.Region) %>% summarize(days_since_50 = max(days_since_50))

confirmed_models <- covid_ets_state %>% group_by(state.Region) %>% do(model = lm(log(new_confirmed_na) ~  log(new_confirmed_lag7_na) -1, data = .))
confirmed_tidy_model <- confirmed_models %>% tidy(model)

seven_day_elasticity_by_state <- data.frame(state = confirmed_tidy_model$state.Region, days_since_50_cases  = as.numeric(days_since_50_by_state$days_since_50), lag_7_elasticity = confirmed_tidy_model$estimate)


cases_7_day_model <- lm(lag_7_elasticity ~ days_since_50_cases,data = seven_day_elasticity_by_state)

seven_day_elasticity_by_state$prediction <-  cases_7_day_model$coefficients[[1]] + cases_7_day_model$coefficients[[2]] * seven_day_elasticity_by_state$days_since_50_cases
...
require(ggrepel)
ggplot(seven_day_elasticity_by_state_no_outliers, aes(x=days_since_50_cases)) +
    geom_point(shape=1, show.legend = FALSE, (aes(y=lag_7_elasticity, color = state))) +    # Use hollow circles
      geom_line(aes( y = prediction)) +
   geom_label_repel(data=  seven_day_elasticity_by_state_no_outliers, aes(x=days_since_50_cases, y=lag_7_elasticity, label = state), size=2) + 
    theme(legend.position = "none") 

knitr::kable(seven_day_elasticity_by_state_no_outliers %>% mutate(residual = lag_7_elasticity - prediction) %>% arrange(days_since_50_cases, residual), caption = "Countries by Predicted vs. Actual Lag 7 New Case Elasticity on Today's Cases")

actuals <- covid_ets_state %>% select(state = state.Region, date, days_since_50_cases = days_since_50, confirmed, new_confirmed, new_confirmed_lag7)

prediction <- expand.grid(state = unique(seven_day_elasticity_by_state$state), days_since_50_cases = 1:100) %>% 
  arrange(state, days_since_50_cases)

prediction$prediction <- predict.lm(cases_7_day_model, prediction)

prediction <- merge(prediction, actuals, on.x = c("state", "days_since_50_cases"), all.x = TRUE, all.y = FALSE)
