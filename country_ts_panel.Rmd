...
confirmed_models <- covid_country_ets %>% group_by(Country.Region) %>% do(model = lm(log(confirmed_na) ~  log(confirmed_lag7_na) -1, data = .))
confirmed_tidy_model <- confirmed_models %>% tidy(model)

seven_day_elasticity_by_country <- data.frame(country = confirmed_tidy_model$Country.Region, days_since_50_cases  = days_since_50_by_country$days_since_50, lag_7_elasticity = confirmed_tidy_model$estimate)

seven_day_elasticity_by_country_no_outliers <- seven_day_elasticity_by_country %>% filter(lag_7_elasticity <2)

require(ggrepel)
ggplot(seven_day_elasticity_by_country_no_outliers, aes(x=days_since_50_cases, y=lag_7_elasticity, color = country)) +
    geom_point(shape=1, show.legend = FALSE) +    # Use hollow circles
...
cases_7_day_model <- lm(log(lag_7_elasticity) ~ days_since_50_cases,data = seven_day_elasticity_by_country_no_outliers)

predict_days <- data.frame(days_since_50_cases = 1:100)

prediction <- predict.lm(cases_7_day_model, predict_days)
