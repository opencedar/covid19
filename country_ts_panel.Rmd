...
for(i in 1:length(unique(prediction_us$state))) {
  ...
  predict$forecast_new <- round(forecast_forward(predict$new_confirmed, (predict$us_coefficient_adj)-.05, 7), digits = 0)
...
forecast_summary <- inner_join(forecast_summary, state_pop, by = c("state" = "State"))
forecast_summary <- forecast_summary %>% select(-state.region, -ages, -year) %>% mutate(perc_pop_infected = total_cases / population)
