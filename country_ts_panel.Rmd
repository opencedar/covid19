...
knitr::kable(forecast_summary, caption= "Forecast Peak New Cases by State", align = c("l","r","r","r","r","r","r","r","r"))
...
ggplot(testing_df_dd, aes(x = date, y= instant_positive_rate, group = state, color = state)) +
  ...
  theme(legend.position = "none") +
  geom_text(data = testing_df_dd %>% group_by(state) %>% filter(date == max(date)), aes(label=state, x = date + 1, y= instant_positive_rate, color = state), size = 2)
